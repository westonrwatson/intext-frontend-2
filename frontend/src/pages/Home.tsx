...
export const Home = () => {
  ...
  const getScrollingMovies = async () => {
    const response = await fetchData({
      path: "titles?countOnly=true&count=150",
    });
  ...
  const getRecentlyAdded = async () => {
    const response = await fetchData({
      path: "singleTitle?title=All American",
    });

    if (response) {
      const titleGenres = Object.keys(response).filter(
        (key) => genres.includes(key) && response[key] === 1
      );
  ...
  const getUserRecommendations = async (userId: string) => {
    const response = await fetchData({
      path: `user-recommendations?user_id=${userId}&genre=Fantasy&genre=Comedies&genre=Dramas&genre=Horror%20Movies`,
    });
    return response;
  };
  ...
  const getAllUserRecommendations = async (userId: string) => {
    const response = await fetchData({
      path: `user-recommendations?user_id=${userId}`,
    });
    return response;
  };
    ...
    const fetchRecommendations = async () => {
      try {
        ...
        if (!Array.isArray(recommendations)) {
          console.error(
            "❌ Invalid response format for recommendations:",
            recommendations
          );
          return;
        }
        ...
        <div className="flex flex-col items-start justify-center w-full h-[calc(80%-20px)] overflow-clip text-white gap-4 relative">
          <img
            ...
            onError={(e) => {
              ...
              target.onerror = null;
              target.src =
                "https://cdn.spotparking.app/public/posters/fallbackImage.jpg";
            }}
          ...
          {recentlyAdded && (
            <div className="absolute bottom-40 left-40 flex flex-col gap-3 z-10">
              ...
              <p className="font-semibold text-6xl text-shadow-lg">
                {recentlyAdded?.title}
              </p>
              <p className="font-light text-md text-gray-200 text-shadow-lg">
                {recentlyAdded?.genres.join(", ")}
              </p>
      ...
      <div className="flex flex-col items-center justify-start bg-[#191919] min-h-screen no-scrollbar w-full pb-10 gap-8 py-0">
        ...
        <div className="flex flex-col items-center justify-center w-full overflow-hidden text-white gap-4 relative">
          <div className="absolute top-1/2 -translate-y-1/2 flex flex-col justify-center items-center gap-6 z-20">
            <p className="text-[55px] text-center text-shadow-lg font-bold text-white select-none">
              Rediscover Film. Curated Classics,
            </p>
            <p className="text-[55px] text-center text-shadow-lg font-bold text-white select-none -mt-10">
              Hidden Gems, Indie Gold.
            </p>
            <a
              href="/sign-up"
              draggable={false}
              className="text-xl text-white bg-[#503047] py-4 px-9 rounded-full font-light hover:bg-[#402639] transition cursor-pointer select-none"
            >
              Start Watching Now
            </a>
          </div>
        ...
        <div
          className="w-full h-fit flex flex-col justify-center items-center bg-[#191919]"
          id="scroll-target"
        >
          <div className="w-2/3 h-fit bg-[#191919] flex flex-col items-center relative py-20">
            <div className="flex flex-row gap-20 justify-center items-center relative">
              <div className="flex flex-col justify-center items-start text-right gap-4 text-white w-1/2  py-64 mr-[500px] pl-10">
                <p className="text-4xl md:text-5xl text-left w-full font-bold">
                  Why CineNiche?
                </p>
                <p className="text-base md:text-lg text-left w-full font-light">
                  At CineNiche, we believe great films shouldn’t be buried by
                  algorithms. We handpick every title, from underground cult
                  hits to award-winning international gems, so every
                  recommendation feels personal.
                </p>
                <p className="text-base md:text-lg text-left font-light">
                  Whether you're a lifelong cinephile or just tired of the same
                  old blockbusters, CineNiche is your passport to a deeper,
            ...
            <div className="flex flex-row gap-20 justify-center items-center relative">
              ...
              <div className="flex flex-col justify-center items-end text-right gap-4 text-white w-3/4  py-36 ml-[500px] pr-10">
                <p className="text-4xl md:text-5xl font-bold">
                  Rediscover the Stories <br /> That Shaped Cinema
                </p>
                <p className="text-base md:text-lg font-light max-w-[500px]">
                  Some of the most powerful films were never made for the
                  spotlight. At CineNiche, we celebrate the overlooked and the
                  unforgettable: cult favorites, indie gems, global cinema, and
                  documentaries that offer something real. Each title is
                  carefully chosen to bring fresh perspective and lasting
                  impact.
                </p>
                <p className="text-base md:text-lg font-light max-w-[450px]">
                  These are the stories worth watching again and again. They
                  challenge how we think, reflect what we feel, and remind us
                  why cinema matters.
                </p>
              </div>
            </div>
          ...
          <div className="w-full h-fit flex flex-col justify-center items-center gap-10 pb-10">
            <div className="flex flex-col gap-1 justify-center items-center py-6">
              <p className="text-4xl md:text-4xl select-none text-center pt-32 text-white w-full font-bold">
                Where You Can Watch
              </p>
            ...
            <div
              draggable={false}
              className="w-[90%] max-w-6xl h-64 bg-[#E9AF59] rounded-xl flex flex-col justify-center items-center gap-8"
            >
              <p
                draggable={false}
                className="text-4xl md:text-5xl text-center text-white w-full font-semibold select-none"
              >
              ...
              <a
                href="/sign-up"
                draggable={false}
                className="text-xl text-white bg-[#503047] py-4 px-8 rounded-full hover:bg-[#402639] transition cursor-pointer select-none"
              >
                Sign Up Now
              </a>
            </div>
